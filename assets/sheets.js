...
(function (global) {
  ...
  Sheets.getLinkForSach1000 = async function (id) {
    ...
    try {
      ...
      if (typeof data?.link === 'undefined' && typeof data?.links === 'undefined') return null;
      const links = Array.isArray(data?.links) ? data.links.filter((s) => String(s || '').trim()) : undefined;
      let payload = { link: (data && data.link) || (links && links[0]) || '', links: links, status: (data && data.status) || undefined };
